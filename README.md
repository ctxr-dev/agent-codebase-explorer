...
A read-only codebase-exploration subagent for the focused-subagent discipline: hand it a self-contained search brief, get back a capped findings report. It is scoped to Read/Grep/Glob/Bash and carries no MCP connector tools, so it stays immune to a bad connector schema that would otherwise kill subagent init.
...
Invoke it as a fan-out subagent during exploration or planning, when you need to locate code without spending main-context tokens:

```text
Agent(subagent_type: "ctxr-agent-codebase-explorer",
      prompt: "<self-contained search brief: what to find, where to look, what to report>")
```

It returns a capped report (exact `file:line` references, what it ruled out, open questions) and never edits anything. Run two or three in parallel with disjoint scopes for broad fan-out.
